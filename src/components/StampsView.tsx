...
export default function StampsView({ locations, onExploreLocation, onToggleCheckIn }: StampsViewProps) {
  ...
  const selectedBadgeTranslated = selectedBadge 
    ? translateLocation(locations.find(l => l.id === selectedBadge.id) || selectedBadge) 
    : null;
      ...
      <div className="bg-surface-container-high/40 border border-[#005049]/15 p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="text-left">
          ...
          <p className="font-headline text-lg font-bold text-on-surface">
            {t('canjeados_label').replace('{completed}', String(unlockedCount)).replace('{total}', String(totalCount))}
          </p>
        </div>
        ...
        <div className="text-left sm:text-right max-w-md">
          <p className="text-xs text-on-surface-variant italic leading-relaxed">
            {unlockedCount === totalCount 
              ...
              : t('album_supremo_missing').replace('{num}', String(totalCount - unlockedCount))
            }
          </p>
        </div>
      </div>
            ...
            <div className="absolute top-4 right-4 z-50">
              ...
              {showShareMenu && (
                <div className="absolute right-0 mt-1.5 w-48 bg-surface-container-highest border border-outline-variant/30 rounded-xl shadow-2xl p-1.5 space-y-0.5 animate-in slide-in-from-top-1 fade-in duration-150 z-50">
                  ...
                  <button
                    onClick={() => handleShare('twitter', selectedBadgeTranslated)}
                  ...
                  <button
                    onClick={() => handleShare('facebook', selectedBadgeTranslated)}
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-[#005049]/15 text-on-surface flex items-center gap-2 transition-colors"
                  >
                  ...
                  <button
                    onClick={() => handleShare('whatsapp', selectedBadgeTranslated)}
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-[#005049]/15 text-on-surface flex items-center gap-2 transition-colors"
                  >
                  ...
                  <button
                    onClick={() => handleShare('copy', selectedBadgeTranslated)}
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-[#005049]/15 text-on-surface flex items-center gap-2 transition-colors"
                  >
            ...
            <div className="space-y-6 text-center pt-4">
              ...
              >
                <div className={`absolute inset-0 rounded-full blur-xl opacity-35 bg-gradient-to-tr ${
                  selectedBadgeTranslated.isCheckedIn ? 'from-secondary to-tertiary' : 'hidden'
                }`} />
                <motion.div 
                  ...
                  animate={selectedBadgeTranslated.isCheckedIn ? {
                ...
                >
                  <img 
                    src={selectedBadgeTranslated.badgeImageUrl || badgeImageMap[selectedBadgeTranslated.badgeIcon] || stampAlhambra} 
                    alt={selectedBadgeTranslated.badgeName} 
                    referrerPolicy="no-referrer"
                    style={{ filter: selectedBadgeTranslated.isCheckedIn ? "url(#remove-white)" : "url(#remove-white) grayscale(100%) opacity(0.35)" }}
              ...
              <div className="space-y-2 flex flex-col items-center">
                <h3 className="font-headline text-lg font-bold text-on-surface">
                  {t('insignia_prefix').replace('{name}', selectedBadgeTranslated.badgeName)}
                </h3>
                ...
                <button
                  onClick={() => {
                     if (onToggleCheckIn) {
                      onToggleCheckIn(selectedBadgeTranslated.id);
                      setSelectedBadge(prev => prev ? { ...prev, isCheckedIn: !prev.isCheckedIn } : null);
                      triggerToast(!selectedBadgeTranslated.isCheckedIn ? '¡Insignia Completada!' : 'Insignia marcada como pendiente');
                    }
                  }}
                  className={`px-3 py-1 rounded-full flex items-center justify-center gap-1 text-[10px] font-bold uppercase tracking-wider transition-all border shrink-0 ${
                    selectedBadgeTranslated.isCheckedIn 
                ...
                >
                  ...
                  <span>{selectedBadgeTranslated.isCheckedIn ? t('listo_caps') : t('marcar_listo')}</span>
                </button>
              </div>
              ...
              <div className="flex gap-2.5">
                ...
                <button
                  onClick={() => {
                    ...
                    setShowShareMenu(false);
                    setToastMessage(null);
                    onExploreLocation(selectedBadgeTranslated.id);
                  }}
